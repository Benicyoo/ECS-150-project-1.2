...
struct cmd_string {
        char *argv[ARG_MAX + 1];
        int argc;
        char original_command[CMDLINE_MAX];
};

//parse command
void parse_command(char *input, struct cmd_string *cmd, int type) { //pass in pointers to the struct and input
        strcpy(cmd->original_command, input); //store original command before tokenizing
        char *x;
...
void redirect(struct cmd_string *cmd){
        if(strchr(cmd->original_command, '>')!=NULL){
                char* output_filename= strtok(cmd->original_command, ">");
...
//MULTIPIPE FUNCTION
int* multipipe(char *command,int* count, int bg_flag, pid_t **bg_pids_output){
        ...
        while(i != *count){
            if(i==0){
                ...
                if(!(pids[i]=fork())){
                        ...
                        if(strchr(commands[i].original_command, '<') != NULL){
...
void monitor_bg() {
        if (!bg_in_process || !bg_process_pids) return;

        int finished = 1;
        int statuses[bg_count];
        ...
        if (bg_output_fd != -1) {
                char buf[1024];
                ssize_t n;
                while ((n = read(bg_output_fd, buf, sizeof(buf))) > 0) {
        ...
        for (int i = 0; i < bg_count; i++) {
                if (WIFEXITED(statuses[i])) {
                        fprintf(stderr, "[%d]", WEXITSTATUS(statuses[i]));
                } 
                else if (WIFSIGNALED(statuses[i])) {
                        fprintf(stderr, "[sig=%d]", WTERMSIG(statuses[i]));
                } 
                else {
                        fprintf(stderr, "[?]");
                }
...
int main(void){
        ...
        while (1) {
                ...
                if(strchr(cmd, '|') != NULL){
                        ...
                        int count = 0;
                        int x = 0;
                        if (!bg_flag) monitor_bg();
                        exitStatus=multipipe(cmd,&count, bg_flag, &bg_pids);
                        if (bg_flag) {
                                store_bg(bg_pids, original_cmd, count);
                                continue;
                        }
                        for(int i = 0; i < count; i++){
                ...
                if(pid > 0){
                        ...
                        if (bg_flag == 1) {
                            ...
                            if (result == 0) {
                                        ...
                                        pidptr[0] = pid;
                                        store_bg(pidptr, original_cmd,1);
                            } else if (result == pid) {
                                        fprintf(stderr, "+ completed '%s' [%d]\n", original_cmd, WEXITSTATUS(status)); //REMOVED & HERE
                            } else {
